...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        let header: [u8; 8] = value[0..0 + Png::STANDARD_HEADER.len()].to_vec().try_into().unwrap();
        i += Png::STANDARD_HEADER.len();
        ...
        loop {
            ...
            let length = u32::from_be_bytes(value[i..i + 4].to_vec().try_into().unwrap());
            match Chunk::try_from(&value[i..i + 12 + length as usize].to_vec()) {
                Ok(ck) => results.push(ck),
                Err(e) => {
                    return Err(e);
                }
            }
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let ck = ChunkType::from_str(chunk_type);
        if ck.is_err() {
            return Err(ck.unwrap_err());
        }
        let chunk_type1 = ck.unwrap();
        let chunk = self.0.iter().position(|chunk| {
            *chunk.chunk_type() == chunk_type1
        });
        match chunk {
            Some(chunk_idx) => {
                let chunk1 = self.0.remove(chunk_idx);
                Ok(chunk1)
            }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk_type_r = ChunkType::from_str(chunk_type);
        if chunk_type_r.is_err() {
            return None;
        }
        let chunk_type1 = chunk_type_r.unwrap();
        self.0.iter().find(|c| {
            *c.chunk_type() == chunk_type1
        })
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut result: Vec<u8> = self.header().iter().copied().collect();
        let vs = self.0.iter().flat_map(|c| c.as_bytes()).collect::<Vec<u8>>();
        result.extend(vs);
        result
        // vs
    }
